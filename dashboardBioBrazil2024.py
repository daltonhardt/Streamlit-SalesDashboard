...
dataframe_header = ['TICKET', 'DATA', 'HORA', 'TIPO-VENDA', 'FORMA-PAGO', 'CODIGO', 'PRODUTO', 'QTDE', 'VALOR-ITEM',
                 'TOTAL-ITEM', 'DESCONTO', 'TOTAL', 'TOTAL-COMPRA', 'TOTAL-COBRADO']
...
if arquivo:
    ...
    if df.columns.values.tolist() != dataframe_header:
        ...
        st.error("O arquivo não contém a estrutura correta de dados! Tente outro.")
        ...
        # calculando percentual entre TOTAL-COBRADO e TOTAL-COMPRA
        df['DIF'] = df['TOTAL-COBRADO'] / df['TOTAL-COMPRA']
        df['DIF'] = df['DIF'].fillna(0)
        print('Header:\n', df.head(15))
        df['TOTAL-AJUSTADO'] = df['TOTAL'] * df['DIF']
        #df['TOTAL-AJUSTADO'] = df['TOTAL-AJUSTADO'].round(2)

        df_total_venda_feira = df.copy()
        df_total_venda_feira = df_total_venda_feira.drop_duplicates(['TICKET'])
        df_total_venda_feira = df_total_venda_feira['TOTAL-COBRADO'].sum()
        ...
        #df_valor_cobrado.reset_index(inplace=True, drop=True)
        #print('df_valor_cobrado  2  :\n', df_valor_cobrado)

        # if not tipoVenda and not formaPago:
        ...
        with col7:
                ...
                df_filtrado_qtde = df_filtrado_qtde.sort_values(['QTDE'], ascending=False)
        ...
        if not (len(tipoVenda) == 1 and 'BRINDE' in tipoVenda):
            with col8:
                ...
                df_filtrado_total_produto = df_filtrado_total_produto.sort_values(['TOTAL-AJUSTADO'], ascending=False)
                st.subheader(f'Total Vendas {filtro}')
                fig = px.bar(df_filtrado_total_produto, x='PRODUTO', y='TOTAL-AJUSTADO', text = ['R$ {:,.2f}'.format(x) for x in df_filtrado_total_produto["TOTAL-AJUSTADO"]],
                            template='gridon', hover_data=['TOTAL-AJUSTADO'])
                st.plotly_chart(fig, use_container_width=True)  
            
        #print('df_filtrado:\n', df_filtrado)
        #st.write(df_filtrado)
        st.dataframe(df_filtrado.style.format(subset=["VALOR-ITEM", "TOTAL-ITEM", "TOTAL", "TOTAL-COMPRA",
                                    "TOTAL-COBRADO", "TOTAL-AJUSTADO"], formatter="{:.2f}"),
                     column_order=("DIA", "TICKET", "TIPO-VENDA", "FORMA-PAGO",
                                    "PRODUTO", "QTDE", "VALOR-ITEM", "TOTAL-ITEM",
                                    "DESCONTO", "TOTAL", "TOTAL-COMPRA", "TOTAL-COBRADO", "DIF", "TOTAL-AJUSTADO"))
